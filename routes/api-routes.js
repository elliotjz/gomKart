...
module.exports = (app, jsonParser) => {
  ...
  app.get("/api/get-tournaments", (req, res) => {
    ...
    const email = getCodeFromQueryString(query, "email");

    Tournament.find({ adminUsers: email }).then(tournaments => {
      const tCompressed = tournaments.map(t => ({
        name: t.name,
        code: t.code
      }));
      res.json({ tournaments: tCompressed });
    });
  });
  ...
  app.post("/api/add-player", jsonParser, (req, res) => {
    ...
    if (verifyName(name)) {
      ...
      Tournament.findOne({ code, adminUsers: email }, (err, tournament) => {
        if (err) {
          ...
          if (playersWithSameName === undefined) {
            ...
            console.log("player with same name");
            res.json({
              error: "There's already a player with this name in the tournament"
            });
          }
        }
      });
  ...
  app.post("/api/change-player-name", jsonParser, (req, res) => {
  ...
  app.post("/api/delete-player", jsonParser, (req, res) => {
  ...
  app.post("/api/add-race", jsonParser, (req, res) => {
    const date = new Date();
    const { places, email, code: tournamentCode } = req.body;
    ...
    Tournament.findOne(
      { code: tournamentCode, adminUsers: email },
      (err, tournament) => {
